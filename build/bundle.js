...
var helpers$2 = {
	...
	valueAtIndexOrDefault: function(value, index, defaultValue) {
		return helpers$2.valueOrDefault(helpers$2.isArray(value) ? value[index] : value, defaultValue);
	},
	...
	each: function(loopable, fn, thisArg, reverse) {
		...
		} else if (helpers$2.isObject(loopable)) {
	...
	arrayEquals: function(a0, a1) {
		...
		for (i = 0, ilen = a0.length; i < ilen; ++i) {
			...
			if (v0 instanceof Array && v1 instanceof Array) {
				if (!helpers$2.arrayEquals(v0, v1)) {
					return false;
				}
			} else if (v0 !== v1) {
	...
	clone: function(source) {
		if (helpers$2.isArray(source)) {
			return source.map(helpers$2.clone);
		}

		if (helpers$2.isObject(source)) {
			var target = {};
			...
			for (; k < klen; ++k) {
				target[keys[k]] = helpers$2.clone(source[keys[k]]);
			}
	...
	_merger: function(key, target, source, options) {
		...
		if (helpers$2.isObject(tval) && helpers$2.isObject(sval)) {
			helpers$2.merge(tval, sval, options);
		} else {
			target[key] = helpers$2.clone(sval);
		}
	},
	...
	merge: function(target, source, options) {
		var sources = helpers$2.isArray(source) ? source : [source];
		var ilen = sources.length;
		var merge, i, keys, klen, k;

		if (!helpers$2.isObject(target)) {
			return target;
		}

		options = options || {};
		merge = options.merger || helpers$2._merger;
	...
	mergeIf: function(target, source) {
		return helpers$2.merge(target, source, {merger: helpers$2._mergerIf});
	},
	...
	extend: function(target) {
		...
		for (var i = 1, ilen = arguments.length; i < ilen; ++i) {
			helpers$2.each(arguments[i], setFn);
		}
		return target;
	},
	...
	inherits: function(extensions) {
		...
		ChartElement.extend = helpers$2.inherits;

		if (extensions) {
			helpers$2.extend(ChartElement.prototype, extensions);
		}
 ...
 * @private
 */
helpers$2.callCallback = helpers$2.callback;
 ...
 * @private
 */
helpers$2.indexOf = function(array, item, fromIndex) {
 ...
 * @private
 */
helpers$2.getValueOrDefault = helpers$2.valueOrDefault;
 ...
 * @private
 */
helpers$2.getValueAtIndexOrDefault = helpers$2.valueAtIndexOrDefault;
...
var element_point = core_element.extend({
	...
	draw: function(chartArea) {
		...
		ctx.lineWidth = helpers.valueOrDefault(vm.borderWidth, core_defaults.global.elements.point.borderWidth);
...
var core_plugin = function(Chart) {
	 ...
	 * @private
	 */
	Chart.PluginBase = core_element.extend({});
};
...
var core_layoutService = function(Chart) {
...
var core_scale = function(Chart) {
	...
	function parseLineHeight(options) {
		return helpers.options.toLineHeight(
			...
			helpers.valueOrDefault(options.fontSize, core_defaults.global.defaultFontSize));
	}
...
var scale_linearbase = function(Chart) {
	...
	Chart.LinearScaleBase = Chart.Scale.extend({
		...
		buildTicks: function() {
			...
			var ticks = me.ticks = core_ticks.generators.linear(numericGeneratorOptions, me);
	...
	var LinearScale = Chart.LinearScaleBase.extend({
		...
		getTickLimit: function() {
			...
			if (me.isHorizontal()) {
				...
				var tickFontSize = helpers.valueOrDefault(tickOpts.fontSize, core_defaults.global.defaultFontSize);
...
var scale_logarithmic = function(Chart) {
	...
	var LogarithmicScale = Chart.Scale.extend({
		...
		buildTicks: function() {
			...
			var ticks = me.ticks = core_ticks.generators.logarithmic(generationOptions, me);
...
var scale_time = function(Chart) {
	...
	var TimeScale = Chart.Scale.extend({
		...
		getLabelWidth: function(label) {
			...
			var tickFontSize = helpers.valueOrDefault(ticksOpts.fontSize, core_defaults.global.defaultFontSize);
...
core_defaults._set('pie', helpers.clone(core_defaults.doughnut));
...
var plugin_legend = function(Chart) {
	...
	Chart.Legend = core_element.extend({
...
var plugin_title = function(Chart) {

	var layout = Chart.layoutService;
	var noop = helpers.noop;

	Chart.Title = core_element.extend({
		initialize: function(config) {
			var me = this;
...
var searchHtml = "<template>\r\n  <tr>\r\n    <td data-for=\"name\">Name</td>\r\n    <td data-for=\"last-ip\">Last IP</td>\r\n    <td data-for=\"last-join\">Last Join</td>\r\n    <td><button class=\"button is-small\" data-do=\"name\">More info</button></td>\r\n  </tr>\r\n</template>\r\n\r\n\r\n<div class=\"container is-widescreen\">\r\n  <br>\r\n  <h3 class=\"subtitle\">Search</h3>\r\n  <input class=\"input\" placeholder=\"Name or IP\">\r\n  <br>\r\n  <hr>\r\n\r\n  <table class=\"table is-fullwidth is-striped is-narrow\">\r\n    <thead>\r\n      <tr>\r\n        <th>Name</th>\r\n        <th>Last IP</th>\r\n        <th>Last join</th>\r\n        <th>Extra</th>\r\n      </tr>\r\n    </thead>\r\n    <tbody></tbody>\r\n  </table>\r\n</div>\r\n";
...
bot.MessageBot.registerExtension('bibliofile/analytics', (ex, world) => __awaiter(undefined, void 0, void 0, function* () {
    console.log('Register');
    const ui = ex.bot.getExports('ui');
    if (!ui)
        return; // Not useful in node bots
    ...
    const logs = yield world.getLogs(true);
    if (!logs.length) {
  ...
  ${niceDate(firstEntry.timestamp)}
    ...
    graph(canvas.getContext('2d'), sortIntoHours(logs), sortIntoHours(lastWeekLogs), sortIntoHours(lastDayLogs));
    // Players tab
    const playerList = Object.keys(results.players)
        .map(name => (Object.assign({}, results.players[name], { name })))
    ...
    playerTab.addEventListener('click', event => {
        ...
        const makeList = (arr) => '<ul><li>' + arr.map(stripHTML).join('</li><li>') + '</li></ul>';
        ui.alert(`
      <div class="content">
...
function analyzeLogs(logs) {
    ...
    logs.forEach(entry => {
        ...
        if (leaveInfo) {
            ...
            if (!players[name])
                return; // Fake leave or join rolled off.
...
function getPlayerInfoFromLeave({ message }) {
    if (!message.includes(' - Player Disconnected '))
        return null;
    return message.substr(message.indexOf(' - Player Disconnected ' + 23));
}
function calcBounceRate(playerData) {
...
function calcNumberDuplicateAccounts(results) {
...
function graph(ctx, all, week, day) {
    new chart(ctx, {
        type: 'line',
        data: {
